...
<template>
  <ToolLayout max-width="4xl">
    <ToolHeader title="PDF 工具" description="压缩、合并、拆分、水印、转图片" :icon="ImageIcon" />
    ...
    <div class="flex flex-wrap gap-2">
      <button
        ...
        :class="activeTab === t.key ? 'bg-primary text-on-primary' : 'bg-surface-variant text-on-surface-variant hover:bg-surface-variant/80'"
      >
        {{ t.label }}
      </button>
    </div>
    ...
    <ToolCard v-if="activeTab === 'compress'">
      <div class="space-y-4">
        <div class="flex items-center gap-3 rounded-2xl border-2 border-dashed border-outline p-4 transition-colors hover:border-primary hover:bg-primary-container/20" @dragover.prevent @drop="handleCompressDrop">
        ...
        <div class="flex flex-wrap gap-2">
          <button v-for="level in [{ key: 'light', label: '轻度' }, { key: 'medium', label: '标准' }, { key: 'strong', label: '强力' }] as const" :key="level.key" @click="compressLevel = level.key" class="rounded-full px-3 py-1.5 text-xs font-medium transition-colors" :class="compressLevel === level.key ? 'bg-secondary-container text-on-secondary-container' : 'bg-surface-variant text-on-surface-variant hover:bg-surface-variant/80'">
            {{ level.label }}
          </button>
        </div>

        <button @click="doCompress" :disabled="!compressFile || compressLoading" class="flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors disabled:opacity-50">
    ...
    <ToolCard v-if="activeTab === 'merge'">
      <div class="space-y-4">
        <div class="flex items-center justify-center">
          <button @click="triggerMergeUpload" @dragover.prevent @drop="handleMergeDrop" class="flex flex-col items-center gap-2 rounded-2xl border-2 border-dashed border-outline p-8 transition-colors hover:border-primary hover:bg-primary-container/30">
        ...
        <div v-if="mergeFiles.length" class="space-y-2">
          <div v-for="(item, idx) in mergeFiles" :key="idx" class="flex items-center gap-2 rounded-xl bg-surface-variant/30 px-3 py-2">
        ...
        <button @click="doMerge" :disabled="mergeFiles.length < 2 || mergeLoading" class="flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors disabled:opacity-50">
          <Merge class="h-4 w-4" />
    ...
    <ToolCard v-if="activeTab === 'split'">
      <div class="space-y-4">
        <div class="flex items-center gap-3 rounded-2xl border-2 border-dashed border-outline p-4 transition-colors hover:border-primary hover:bg-primary-container/20" @dragover.prevent @drop="handleSplitDrop">
          <label class="flex cursor-pointer items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors">
            <Upload class="h-4 w-4" />
            选择 PDF
        ...
        <div>
          ...
          <input v-model="splitRanges" placeholder="如 1-3,5,7-10" class="h-10 w-full rounded-lg border border-outline bg-transparent px-3 text-sm text-on-surface outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
          <div class="mt-1 text-xs text-on-surface-variant">用逗号分隔，用短横线表示范围</div>
        </div>

        <button @click="doSplit" :disabled="!splitFile || splitLoading" class="flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors disabled:opacity-50">
    ...
    <ToolCard v-if="activeTab === 'watermark'">
      <div class="space-y-4">
        <div class="flex items-center gap-3 rounded-2xl border-2 border-dashed border-outline p-4 transition-colors hover:border-primary hover:bg-primary-container/20" @dragover.prevent @drop="handleWmDrop">
          <label class="flex cursor-pointer items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors">
            <Upload class="h-4 w-4" />
            选择 PDF
        ...
        <button @click="doWatermark" :disabled="!wmFile || wmLoading" class="flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors disabled:opacity-50">
          <Type class="h-4 w-4" />
    ...
    <ToolCard v-if="activeTab === 'toImage'">
      <div class="space-y-4">
        <div class="flex items-center gap-3 rounded-2xl border-2 border-dashed border-outline p-4 transition-colors hover:border-primary hover:bg-primary-container/20" @dragover.prevent @drop="handleTiDrop">
          <label class="flex cursor-pointer items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors">
            <Upload class="h-4 w-4" />
            选择 PDF
        ...
        <button @click="doToImage" :disabled="!tiFile || tiLoading" class="flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-on-primary shadow-sm hover:bg-primary/90 transition-colors disabled:opacity-50">
